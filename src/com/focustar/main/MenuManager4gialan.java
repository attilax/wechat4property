 ...
 */
public class MenuManager4gialan {

	private static Logger log = Logger.getLogger(MenuManager4gialan.class);
	...
	Menu	me =getMenu();
	String jsonMenu = JSONObject.fromObject(me).toString();
	core.log(jsonMenu);
	filex.save(jsonMenu, pathx.webAppPath()+"\\gialenMenu.json");
		 if(jsonMenu.length()>0)
			 return;
		
		
	String TrueCfg=	Constant.path + "WeiXinConfig4gialen.properties";
		...
		// 第三方用户唯一凭证
		appId = configService.getWxProperty("APPID",TrueCfg);
		// 第三方用户唯一凭证密钥
		appSecret = configService.getWxProperty("APPSECRET",TrueCfg);

		webSite = configService.getWxProperty("NEWS_WEBSITE",TrueCfg);
		// 调用接口获取access_token
...
//		btn31.setKey("31");
		...
		ViewButton skrechCard = new ViewButton("9周年刮刮卡抽奖,view," +
		...
		mainBtn3.setSub_button(new Button[] {  btn33,微信价});
