...
import com.focustar.entity.weixin.sendMessage.BaseMessage;
import com.focustar.entity.weixin.sendMessage.Image;
...
import com.focustar.entity.weixin.sendMessage.Video;
import com.focustar.entity.weixin.sendMessage.VideoMessage;
import com.focustar.entity.weixin.sendMessage.Voice;
...
import com.focustar.util.MyX509TrustManager;
...
public class WeixinUtil {
	 ...
	 * @return
	 */
	public static int createMenu_jsonFile(String menu, String accessToken) {
		int result = 0;
		...
		// 调用接口创建菜单
		JSONObject jsonObject = httpsRequest(url, "POST", jsonMenu);
		core.log(jsonObject.toString(2));

		if (null != jsonObject) {
			if (0 != jsonObject.getInt("errcode")) {
				result = jsonObject.getInt("errcode");
				logger.error("创建菜单失败 errcode:{} errmsg:{} 2" + jsonObject.getInt("errcode") + " " + jsonObject.getString("errmsg"));
			}
		}
	...
	public static UserList getUserList(String accessToken, String nextOpenid) {
		...
		if (jsonObject != null) {
			...
			if (jsonObject.containsValue("errcode")) {
				if (0 != jsonObject.getInt("errcode")) {
					logger.error("获取关注者列表失败 errcode:{} errmsg:{} " + jsonObject.getInt("errcode") + " " + jsonObject.getString("errmsg"));
					return null;
				}
			}
	...
	public static UserInfo getUserInfo(String accessToken, String openid) {
		...
		if (jsonObject != null) {
			...
			if (jsonObject.containsValue("errcode")) {
				if (0 != jsonObject.getInt("errcode")) {
					logger.error("获取用户信息失败 errcode:{} errmsg:{} " + jsonObject.getInt("errcode") + " " + jsonObject.getString("errmsg"));
					return null;
				}
			}else{
	...
	public static String upload(String accessToken, String type, String filePath) {
		...
		builder.append(uploadUrl).append(accessToken).append("&type=").append(type);
	...
	/*public static int getDistance(double lon1, double lat1, double lon2, double lat2){
		...
		double radLat1 = getRad(lat1);
		double radLat2 = getRad(lat2);
		double radLon1 = getRad(lon1);  
        double radLon2 = getRad(lon2);  
	...
	/*public static int getDistance(double lon1, double lat1, double lon2, double lat2){
		...
		double x = getRad(lon1);  
        double y = getRad(lat1);  
        double a = getRad(lon2);  
        double b = getRad(lat2); 
	...
	public static TextMessage buildTextMessage(String openId,String content){
		if(openId != null && !"".equals(openId)){
			com.focustar.entity.weixin.sendMessage.TextMessage textMsg = new com.focustar.entity.weixin.sendMessage.TextMessage();
			Text txt = new Text();
	 ...
	 * @return
	 */
	public static NewsMessage buildLocationMessage(String openId,List<TMbNews> newsList,TMbNews bestNews){
		
		
		if(openId != null && !"".equals(openId) && newsList != null && newsList.size() > 0){
			
			//构建图文消息
			com.focustar.entity.weixin.sendMessage.NewsMessage clientMsg = new com.focustar.entity.weixin.sendMessage.NewsMessage();
			clientMsg.setTouser(openId);
			clientMsg.setMsgtype(MessageUtil.RESP_MESSAGE_TYPE_NEWS);
			News news = new News();
			List<com.focustar.entity.weixin.sendMessage.Article> artList = new ArrayList<com.focustar.entity.weixin.sendMessage.Article>();
			
			//这是固定的，以后要修改，需要到指定目录信息这里面的规则图片
			com.focustar.entity.weixin.sendMessage.Article topArt = new com.focustar.entity.weixin.sendMessage.Article();
			topArt.setTitle("【附近门店】");
			topArt.setDescription("");
			StringBuilder newsUrl = new StringBuilder();
			if(bestNews != null){
				newsUrl.append(Constant.NEWS_WEB_SITE).append(Constant.NEWS_HTML_PATH).append(bestNews.getHtmlName());
			}
			topArt.setUrl(newsUrl.toString());
			StringBuilder topPicUrl = new StringBuilder();
			topPicUrl.append(Constant.WEB_URL).append("weixin/images/sign/daysigntop.jpg");
			...
			smallPicUrl.append(Constant.WEB_URL).append("weixin/images/sign/loc");
			int index = 0;
			
			for(TMbNews oneNews:newsList){
				++index;
				com.focustar.entity.weixin.sendMessage.Article oneArt = new com.focustar.entity.weixin.sendMessage.Article();
	 ...
	 * @return
	 */
	public static NewsMessage builderSignMessage(String openId,List<TMbNews> newsList){
		...
		if(openId != null && !"".equals(openId) && newsList != null && newsList.size() > 0){
			...
			ruleUrl.append(Constant.WEB_URL).append("weixin/mobile/daysignrules.jsp");
	...
	public static NewsMessage buildNewsMessage(String openId,List<TMbNews> newsList){
		...
		List<com.focustar.entity.weixin.sendMessage.Article> artList = new ArrayList<com.focustar.entity.weixin.sendMessage.Article>();
			for(TMbNews oneNews:newsList){
				...
				url.append("https://open.weixin.qq.com/connect/oauth2/authorize?appid=").append(Constant.APPID).append("&redirect_uri=");
				
				
				url.append(Constant.WEB_URL).append("weixin/").append("readServlet?")
					.append("newsId=").append(oneNews.getId());
				//无法获取
					//.append("&newsType=").append(oneNews.getNewsType()).append("&htmlName=").append(oneNews.getHtmlName());
	...
	public static BaseMessage buildMediaMessage(String openId,String mediaId,String msgType){
