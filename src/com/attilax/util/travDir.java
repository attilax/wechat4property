...
public class travDir {
	 ...
	 * @return
	 */
	public static List getAllFileListByRExpress(String strPath, String files_re) {
		// attilax 老哇的爪子  2_f_43   o9l 
		List li = new ArrayList();
		File dir = new File(strPath);
		File[] files = dir.listFiles();
		...
		for (int i = 0; i < files.length; i++) {
			String absolutePath = files[i].getAbsolutePath();
			if (files[i].isDirectory()) {
				List li_t = getAllFileListByRExpress(absolutePath, files_re);
				li.addAll(li_t);
			} else {
				String strFileName = absolutePath;//.toLowerCase();
				
				 //;
				if (strUtil.	isFind_ByWildcard(strFileName,files_re)) {
					// System.out.println("---"+strFileName);
					// filelist.add(files[i].getAbsolutePath());
					li.add(absolutePath);
				}
